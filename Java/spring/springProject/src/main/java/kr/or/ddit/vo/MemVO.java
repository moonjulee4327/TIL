...
/* - 입력값 검증(서버_비즈니스 로직)
	스프링 MVC는 Bean Validation 기능을 잉여해 요청 파라미터 값이
	바인딩 된 도메인 클래스의 입력값 검증을 함
	크롬 -> jsp(post) 요청 -> 자바빈으로 파라미터 매핑 -> 자바빈에서 validation체킹
	-> 오류 발생 시 요청된 페이지 jsp로 forwarding -> jsp에서 오류 표시
	
	- 입력값 검증 규칙 : 입력값 검증 규칙은 Bean Validation이 제공하는 제약 어노테이션으로 설정
...
public class MemVO {
	...
	// 문자열이 null이 아니고 trim(공백제거)한 길이가 3보다 작은 것 검사
	@NotBlank
	@Size(max = 3)
