...
<!-- 
	session기본(내장) 객체
	1. 세션을 사용한다는 것은 session 기본객체를 사용한다는 의미
	2. session객체는 속성(attribute)을 제공함.
		- setAttribute() : 속성에 값 set 메소드
		- getAttribute() : 속성의 값 get 메소드
	3. 웹 브라우자(크롬, 엣지, 파이어폭스, 웨일, 오페라)마다 별도의 세션을 갖음
	4. 웹 브라우저마다 각 세션을 구분하기 위해 세션마다 고유 ID를 할당(세션ID)
	5. 1) 웹 서버(톰켓)는 웹 브라우저(크롬)에 세션ID르 전송
	   2) 웹 브라우저는 웹서버에 요청시 마다 매번 세션 ID를 버내서(쿠키에 넣어서)
	   	  웹 서버(톰켓)가 어떤 세션(웹 브라우저)을 사용할지 판단하 수 있게 함
	6. 웹 서버는 세션ID를 이용하여 웹 브라우저를 위한 세션을 찾음
...
-->
	...
	<%
		...
		String value;
		// 세션객체에 담겨있는 모든 속성 명을 열거형 타입으로 가져옴
